<article class="border {{ $selectedEvent && $selectedEvent->id === $event->id ? 'border-amber-400/60' : (($dashboardSessionActiveEventId ?? null) === $event->id ? 'border-emerald-400/50' : ($event->is_active ? 'border-cyan-400/50' : 'border-slate-800/80')) }} bg-slate-950/65 p-3">
    @php
        $eventStarted = $event->hasStarted();
        $sessionActive = ($dashboardSessionActiveEventId ?? null) === $event->id;
    @endphp
    <div class="flex items-center justify-between gap-2">
        ...
        <div class="flex flex-wrap items-center justify-end gap-1">
            ...
            @endif
            @if ($event->is_active)
                <span class="type-label border border-cyan-400/60 bg-cyan-500/10 px-2 py-0.5 text-[9px] text-cyan-100">Live</span>
            @endif
        </div>
    </div>
    ...
    <div class="mt-3 flex flex-wrap gap-2">
        ...
        @if (in_array($event->status, ['upcoming', 'finished'], true))
            <form action="{{ route('events.live.toggle', $event) }}" method="POST">
                @csrf
                <input type="hidden" name="dashboard_redirect" value="1">
                <input type="hidden" name="dashboard_panel" value="events">
                <input type="hidden" name="dashboard_event_id" value="{{ $event->id }}">
                <button class="type-label border {{ $event->is_active ? 'border-cyan-400/60 bg-cyan-500/10 text-cyan-100 hover:bg-cyan-500/20' : 'border-fuchsia-500/60 text-fuchsia-100 hover:bg-fuchsia-500/10' }} px-2.5 py-1 text-[10px] transition">
                    {{ $event->is_active ? 'Stop Live' : 'Go Live' }}
                </button>
            </form>
        @endif
