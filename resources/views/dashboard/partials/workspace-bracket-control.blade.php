<article class="border border-slate-800/80 bg-slate-950/55 p-2.5">
    @php
        ...
        $nextGateLabel = match (true) {
            ...
            default => 'Generate next elimination round',
        };
        $nextGateDetail = match (true) {
            ...
            default => 'The next elimination round can be generated now.',
        };
    @endphp
            ...
            <div class="flex flex-wrap items-start justify-between gap-2">
                ...
                <div class="flex flex-wrap gap-1.5">
                    <form action="{{ route('events.bracket.generate', $selectedEvent) }}" method="POST">
                        ...
                        <button class="border border-emerald-500/60 bg-emerald-500/10 px-2.5 py-1 text-[8px] font-semibold uppercase tracking-[0.14em] text-emerald-100 transition hover:bg-emerald-500/20">Generate</button>
                    </form>
                    ...
                    @endif
                    <a href="{{ route('dashboard', ['panel' => 'events', 'event' => $selectedEvent->id]) }}" class="border border-slate-700 px-2.5 py-1 text-[8px] font-semibold uppercase tracking-[0.14em] text-slate-100 transition hover:border-cyan-400 hover:text-cyan-200">Edit</a>
                </div>
            </div>
    ...
    <div class="mt-2.5 space-y-2">
                ...
                <div class="mt-2 overflow-x-auto pb-1">
                    <div class="flex min-w-max items-start gap-2.5">
                        @foreach ($swissRounds as $round)
                            <section class="w-[15rem] shrink-0 border border-slate-800/80 bg-slate-950/82">
                        ...
                        @if ($showSwissWaitingColumn)
                            <section class="flex min-h-[14rem] w-[15rem] shrink-0 flex-col border border-dashed border-slate-700/80 bg-slate-950/55">
                        ...
                        @elseif ($showTopCutWaitingColumn)
                            <section class="flex min-h-[14rem] w-[15rem] shrink-0 flex-col border border-dashed border-amber-500/45 bg-amber-500/[0.05]">
        ...
        @if ($eliminationRounds->isNotEmpty())
            <section class="border border-amber-400/20 bg-slate-950/62 p-2.5">
                ...
                <div class="mt-2 grid gap-2 lg:grid-cols-2 2xl:grid-cols-3">
                    @foreach ($eliminationRounds as $round)
                        <section class="border border-slate-800/80 bg-slate-950/78 p-2">
                            <div class="flex items-center justify-between gap-2 border-b border-slate-800/80 pb-2">
                                <p class="text-[12px] font-semibold text-slate-100">{{ $round->label ?: 'Round '.$round->round_number }}</p>
                                <span class="rounded border {{ $round->status === 'completed' ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-200' : 'border-amber-500/40 bg-amber-500/10 text-amber-200' }} px-2 py-1 text-[9px] uppercase tracking-[0.14em]">{{ $round->status }}</span>
                            </div>
    ...
    <div data-workspace-match-modal class="fixed inset-0 z-50 hidden items-center justify-center bg-black/70 p-4">
